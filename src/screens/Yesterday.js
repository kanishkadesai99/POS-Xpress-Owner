...
export default function YesterdayScreen() {
  ...
  const hotelId = route.params?.hotelid ?? 0;      // <-- use the passed hotel id
  const login_id = route.params?.login_id ?? 0;    // (optional, for back nav)

  const [accounts, setAccounts] = useState([]);
  const [selectedAccount, setSelectedAccount] = useState(''); // keep as string for URLs
    ...
    fetch(`https://rnsoftwares.com/XpressOwnerAndroid/get_all_account.php?hotel_id=${hotelId}`)
      ...
      .then(data => {
        ...
        const defaultAcc = result.find(
          acc => String(acc?.AccountName || '').trim().toUpperCase() === 'CASH COUNTER'
        );
  ...
  // Fetch YESTERDAY data when account changes
  useEffect(() => {
    if (!selectedAccount || !hotelId) return;

    setLoading(true);

    // Use yesterday endpoints
    const incomeURL  = `https://rnsoftwares.com/XpressOwnerAndroid/get_income_yesterday.php?hotel_id=${hotelId}&ac_id=${selectedAccount}`;
    const expenseURL = `https://rnsoftwares.com/XpressOwnerAndroid/get_exp_yesterday.php?hotel_id=${hotelId}&ac_id=${selectedAccount}`;
    const summaryURL = `https://rnsoftwares.com/XpressOwnerAndroid/get_summary_yesterday.php?hotel_id=${hotelId}&ac_id=${selectedAccount}`;
    ...
    Promise.allSettled([
      ...
      .catch(err => console.warn('Data fetch failed:', err))
  ...
  const handleAccountSelect = () => {
    ...
    if (Platform.OS === 'ios') {
      const options = accounts.map(acc => String(acc.AccountName));
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.headerRow}>
          ...
          <Text style={styles.title}>YESTERDAY'S DAYBOOK</Text>
        </View>
        ...
        <TouchableOpacity style={styles.fakeInput} onPress={handleAccountSelect} disabled={!accounts.length}>
          <Text style={styles.fakeInputText}>
            {accounts.length === 0
              ? 'Loading...'
              : String(
                  accounts.find(acc => String(acc.AccountId) === String(selectedAccount))?.AccountName ||
                  'Select Account'
                )}
          </Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  rowHeader: {
    flexDirection: 'row', justifyContent: 'space-between', backgroundColor: '#e0e0e0',
    paddingVertical: 6, borderBottomWidth: 1, borderColor: '#bbb',
  },
